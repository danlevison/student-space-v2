    ...
    const toggleSelectedTable = (e) => {
        ...
        const updatedStudentData = studentData.map((student) => {
            if(student.tableData?.tableName === selectedTable) {
                return {...student, tableData: {...student.tableData, selected: !student.tableData.selected}}
            }
            return student
        })
        ...
        try {
            const updatedStudentData = studentData.map((student) => {
                if(student.tableData.selected) {
                    return {...student, tableData: {...student.tableData, tablePoints: 0, selected: false}}
                }
                return student
            })
    ...
    const selectAllTables = () => {
        setStudentData((prevStudentData) => {
            return prevStudentData.map((student) => {
                return {...student, tableData: {...student.tableData, selected: true}}
            })
        })
  ...
  return (
    <>
        <div className="grid grid-cols-[repeat(auto-fill,minmax(130px,1fr))] gap-1 overflow-auto h-[600px]">
            {tables.map((table) => (
                <div key={table.tableName} className="mx-auto h-fit my-2">
                    <input 
